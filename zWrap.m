...
addpath(genpath(fullfile("boards", zSettings.board)));
...
extensionsBasePath = fullfile("boards", zSettings.board, "extensions");

if isfolder(extensionsBasePath)

    for j=1:numel(zSettings.extensions)
        extName = zSettings.extensions{j};
        % Get extension path
        extPath = fullfile(extensionsBasePath, extName);
        ...
        % Run I/O extension
        [iss, oss] = feval(sprintf("%sExtension", extName), "io", iss, oss);
    end
end
