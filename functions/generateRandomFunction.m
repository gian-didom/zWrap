...
% Select randomly inputs to return as outputs
numOutput = randi(numInputs);
outputNames = inputNames(randi(numInputs, [1 numOutput]));

fullOutputCells = vertcat( ...
    sprintf("function [%s] = %s(%s)", strjoin(outputNames, ','), fcnName, strjoin(inputNames, ',')), ...
    fullOutputCells, ...
    "end" ...
    );
...
fprintf("Generated function <strong>%s</strong>.", fcnName);
end
