function [target_input_sizes_arm, target_output_sizes_arm] = getSizeAndOffsetsCpp(buildInfo, codeInfoStruct, packDir)
...
for j=1:numel(typesMap.HostTypes)
    if numel(typesMap.HostTypes(j).Name) > 0
        fprintf("Host type %s is mapped to target type %s\n", ...
            typesMap.HostTypes(j).Name, ...
            codeInfo.Types(typesMap.TargetTypes(j)).Name);
    end
end
...
sourceLines{end+1} = sprintf("} FULL_INPUT_STRUCT;\n");
...
system(sprintf('%s -I %s -c getSizeAndOffsetsGDB.cpp -o getSizeAndOffsetsGDB.o -g', arm_compiler_path, packDir));

%% GET INFO WITH MYTICAL GDB FOR ARM - NO EMULATION NEEDED
...
for j=1:numel(target_input_sizes_arm)
    ...
    [~, outstring] = system(sprintf("%s -q getSizeAndOffsetsGDB.o --ex 'ptype /o _%s' --ex exit", gdb_path, target_input_types(j)));
    fprintf(outstring);
end
...
for j=1:numel(target_input_sizes_arm)
    [~, outstring] = system(sprintf('%s -S getSizeAndOffsetsGDB.o | grep _%s', nm_path, target_input_types(j)));
    splitOut = split(outstring);
    target_input_sizes_arm(j) = hex2dec(splitOut{2});
end

for j=1:numel(target_output_sizes_arm)
    [~, outstring] = system(sprintf('%s -S getSizeAndOffsetsGDB.o | grep _%s', nm_path, target_output_types(j)));
    splitOut = split(outstring);
