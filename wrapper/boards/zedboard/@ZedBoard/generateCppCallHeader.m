...
function CppScript = generateCppCallHeader(obj, save_file_path)
    ...
    for j=1:numel(obj.project.inputs.Children)
        % Parse here the template
        CppScript = vertcat(CppScript, ...
        obj.project.inputs.Children(j).getCppDeclaration);
    end
    ...
    for j=1:numel(obj.project.outputs.Children)
        % Parse here the template
        CppScript = vertcat(CppScript, ...
        obj.project.outputs.Children(j).getCppDeclaration);
    end
