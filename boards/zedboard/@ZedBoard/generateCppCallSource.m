...
function CppScript = generateCppCallSource(obj, save_file_path)
    ...
    CppScript = vertcat(CppScript, ...
        sprintf("%s_initialize(&RTM);", obj.project.inputs.headerName));
    ...
    call_prototype_name = sprintf("%s(", obj.project.inputs.fcnName);
    
    full_args = [obj.project.inputs.Children, obj.project.outputs.Children];
    
    switch (obj.project.inputs.Coder)
        case 'Simulink'
